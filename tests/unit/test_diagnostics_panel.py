...
def _panel_with_rows(rows: list[list]) -> DiagnosticsPanel:
    ...
    panel._store = Gtk.ListStore(str, int, str, str, str)  # type: ignore[call-arg]
    for r in rows:
...
def _row_uris(panel: DiagnosticsPanel) -> list[str]:
    return [row[4] for row in panel._store]


def test_clear_for_uri_drops_only_matching_rows() -> None:
    panel = _panel_with_rows([
        ["Error",   1, "py error",   "pyflakes",  "file:///a.py"],
        ["Warning", 2, "c warn",     "clang",     "file:///b.c"],
        ["Error",   3, "py error 2", "pyflakes",  "file:///a.py"],
        ["Info",    4, "c info",     "clang",     "file:///b.c"],
    ])
...
def test_update_for_uri_replaces_existing_rows_for_that_uri() -> None:
    panel = _panel_with_rows([
        ["Error", 1, "old1", "src", "file:///a.py"],
        ["Error", 2, "old2", "src", "file:///a.py"],
        ["Error", 3, "keep", "src", "file:///b.c"],
    ])
    ...
    assert len(rows) == 2
    # b.c row preserved; a.py row replaced with the single new diagnostic
    assert {r[4] for r in rows} == {"file:///a.py", "file:///b.c"}
    a_row = next(r for r in rows if r[4] == "file:///a.py")
    assert a_row[0] == "Warning"
    assert a_row[1] == 10  # line is 1-based in the panel
    assert a_row[2] == "new1"
